...
mkdir -p ./cockroach
wget -q -O cockroach/cockroach https://edge-binaries.cockroachdb.com/cockroach/cockroach.linux-gnu-amd64.LATEST
...
for test_file in ${COCKROACH_TEST_CASES[@]}; do
    ...
    if ! (RUBYOPT="-W0" TESTOPTS="-v" TEST_FILES="$test_file" bundle exec rake test); then
...
for test_file in ${AR_TEST_CASES[@]}; do
    echo "Running: $test_file"
    if ! (RUBYOPT="-W0" TESTOPTS="-v" TEST_FILES_AR="$test_file" bundle exec rake test); then
        echo "Failed: $test_file"
        HAS_FAILED=1
    else
        echo "Passed: $test_file"
